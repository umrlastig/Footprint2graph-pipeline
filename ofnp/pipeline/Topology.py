...
from ofnp import skeleton_smoothing, process_node, snap_lines_to_connect



def addTopologyToNetwork(RESPATH, SEARCH, h=10, prefix='PT'):
    ...
    for track in progressbar.progressbar(collection):
        track = skeleton_smoothing(track, 1, 5)
    ...
    #          SNAPPING
    #
    collection = snap_lines_to_connect(collection)
    ...
    output_file = str(RESPATH) + 'network/squelette_topology_simplifie_PT.csv'
    tkl.Topology.create_topology(input_file, '2154', output_file)
    ...
    while True and cpt <= len(network.NODES):
        ...
        r = process_node(network, nid, SEARCH, h)
        if r is None:
            HC.append(nid)
